...
export const usePayer = () => Array
	.from( document.querySelectorAll( ACCT_INFO_SELECTOR ) )
	.reduce(
		( acc, p ) => {
...
export const useParseTxns = ( startDateStr?: string, endDateStr?: string ) => {
	...
	document.querySelectorAll( TXNS_SELECTOR )
		.forEach( dateBucket => {
			...
			dateBucket.querySelectorAll( TXN_SELECTOR )
				.forEach( txnEl => {
...
export const useGenerateCsvData = ( txns: Txn[] ): string => txns
	.map( txn => (
		[
			txn.txnDate.toDateString(),
			txn.payer,
			txn.description,
			txn.amount,
		]
...
export const getCsvFileName = (
	payer: string,
) => `transactions-${ payer.replace( /\s/g, '-' ) }-${ Date.now() }.csv`;
